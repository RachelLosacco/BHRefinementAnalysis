# Fourier inflow at constant time (snapshot) as a function of radius
...
radius = np.arange(0.1, 10, 0.1) # kpc
...
def get_info(runtype, version, snapnum, n, pixels, fov, number_points, constant_radius):
    ...
    return nobh, a0, a2, a2/a0, mass_flow
...
for galaxy in galaxies:
    ...
    for r in radius:
        print('radius', r)
        nobh, a0, a2, a2a0, mass_inflow = get_info(runtype, version, snapnum, galaxy, pixels, fov, number_points, r)
        if nobh == True:
            print('no bh')
            a0s.append(a0)
            a2s.append(a2)
    ...
    ax.plot(radius, a2a0s)
    ax.set_xlabel('Distance from BH [kpc]')
    ax.set_ylabel('|A2/A0|')
    ax.set_xscale('log')
    fig.savefig('fourier_radius_a2a0_gal'+str(galaxy)+'_'+version+'_snap'+str(snapnum)+'.png', bbox_inches='tight')
    plt.close()
    
    fig, ax = plt.subplots(figsize=(5,4))
    ax.plot(radius, mdots)
    ax.set_xlabel('Distance from BH [kpc]')
    ax.set_ylabel('dM/dt [kg/(m2 s)]')
    ax.set_xscale('log')
    fig.savefig('fourier_radius_mdot_gal'+str(galaxy)+'_'+version+'_snap'+str(snapnum)+'.png', bbox_inches='tight')
    plt.close()
